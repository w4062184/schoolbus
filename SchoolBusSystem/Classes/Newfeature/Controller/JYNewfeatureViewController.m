...
- (void)viewDidLoad
{
    ...
    // 2.添加图片到scrollView中
    CGFloat scrollW = scrollView.xmg_width;
    CGFloat scrollH = scrollView.xmg_height;
    for (int i = 0; i<HWNewfeatureCount; i++) {
        UIImageView *imageView = [[UIImageView alloc] init];
        imageView.xmg_width = scrollW;
        imageView.xmg_height = scrollH;
        imageView.xmg_y = 0;
        imageView.xmg_x = i * scrollW;
        // 显示图片
...
//    scrollView.bounces = NO; // 去除弹簧效果
    ...
    pageControl.currentPageIndicatorTintColor = JYCommonColor;
    pageControl.pageIndicatorTintColor = JYColor(189, 189, 189);
    pageControl.xmg_centerX = scrollW * 0.5;
    pageControl.xmg_centerY = scrollH - 50;
...
{
    double page = scrollView.contentOffset.x / scrollView.xmg_width;
    // 四舍五入计算出页码
...
{
    if (scrollView.contentOffset.x > scrollView.xmg_width * (HWNewfeatureCount - 1)) {
        [self startClick];
    }
}
...
- (void)setupLastImageView:(UIImageView *)imageView
{
    ...
    startBtn.xmg_width = 200;
    startBtn.xmg_height = 30;
    startBtn.xmg_centerX = imageView.xmg_width * 0.5;
    startBtn.xmg_centerY = imageView.xmg_height * 0.75;
...
- (void)dealloc
{
    JYLog(@"JYNewfeatureViewController-dealloc");
}
